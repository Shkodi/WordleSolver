...
async function testAlgo() {
    ...
    for(index = 0; index < answerLen; index++) {
        ...
        for(counter = 0; counter < numberOfTries; counter++) {    
            ...
            else if(counter === 1 && matchedWords.length === 858 || 
                counter === 1 && matchedWords.length === 581) {
                    currentWord = "colin";
            }
            else if(counter === 1 && matchedWords.length === 814) {
                    currentWord = "lound";
            }
            else if(counter === 1 && matchedWords.length === 637) {
                currentWord = "doily";
            }
            else {
                informationEntropy();
                await informationEntropy();
